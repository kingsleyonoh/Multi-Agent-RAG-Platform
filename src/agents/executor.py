...
Usage::
    ...
    executor = AgentExecutor(registry=registry, max_steps=5)
...
class AgentExecutor:
    ...
    def __init__(self, registry, max_steps: int = 5) -> None:
    ...
    async def run(
        ...
        *,
        ...
        for step in range(self._max_steps):
            ...
            messages.append(choice)  # Add assistant message with tool calls
            ...
            for tc in tool_calls:
                ...
                if not self._registry.is_whitelisted(func_name):
                    ...
                    try:
                        result = await tool_spec.handler(**func_args)
                        result_str = json.dumps(result) if not isinstance(result, str) else result
                    except Exception as exc:
                        result_str = f"Error executing {func_name}: {exc}"
                        logger.error("tool_execution_error", tool=func_name, error=str(exc))
        ...
        return ExecutorResult(
            answer=f"Reached max steps ({self._max_steps}). Could not produce a final answer.",
            tool_calls=tool_call_log,
            total_steps=steps,
        )
